...
.header-band {{
  width:100%;
  background: linear-gradient(90deg, {PRIMARY}, {SECOND});
  padding:22px 0;
  box-shadow: 0 6px 20px rgba(11,43,54,0.14);
  position: sticky;
  top: 0;
  z-index: 999;
  display:flex;
...
for c in ["Total_2022","Total_2023","Taxa_100mil_Mulheres_SINESP","Feminicidios_SSP",
          "Processos_Ajuizados_CNJ","Condenacoes_CNJ","Arquivamentos_CNJ","Indice_Impunidade",
          "Medidas_Protetivas_Concedidas","Atendimentos_Saude_Violencia","Chamadas_180_Total","Variacao_Percentual"]:
...
def standard_layout(fig, height, left_margin=220):
    ...
    fig.update_xaxes(showgrid=False, zeroline=False, title_font=dict(color=PLOT_TEXT), tickfont=dict(color=PLOT_TEXT, size=13))
    fig.update_yaxes(showgrid=False, zeroline=False, title_font=dict(color=PLOT_TEXT), tickfont=dict(color=PLOT_TEXT, size=14), automargin=True, ticks="outside")
    return fig
...
with col1:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=db["Total_2023"], y=db["UF"], orientation="h", marker=dict(color=PRIMARY), hovertemplate="<b>%{y}</b><br>Total: %{x:,}<extra></extra>", name="Total denúncias"))
    scale = db["Total_2023"].max()/max(1,db["Taxa_100mil_Mulheres_SINESP"].max())
    fig.add_trace(go.Scatter(x=db["Taxa_100mil_Mulheres_SINESP"]*scale, y=db["UF"], mode="markers", marker=dict(color=ACCENT,size=9), customdata=np.round(db["Taxa_100mil_Mulheres_SINESP"],2), hovertemplate="<b>%{y}</b><br>Taxa /100k: %{customdata}<extra></extra>", name="Taxa /100k"))
    fig = standard_layout(fig, H_LARGE, left_margin=240)
    fig.update_layout(yaxis=dict(autorange="reversed"), xaxis_title="Total denúncias (escala)")
...
with col2:
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(y=top8.index, x=top8["Chamadas_180_Total"], orientation="h", name="Chamadas 180", marker=dict(color="#9fb8c9"), hovertemplate="%{y}<br>Chamadas: %{x:,}<extra></extra>"))
    fig2.add_trace(go.Bar(y=top8.index, x=top8["Atendimentos_Saude_Violencia"], orientation="h", name="Atendimentos saúde", marker=dict(color=SECOND), hovertemplate="%{y}<br>Atendimentos: %{x:,}<extra></extra>"))
    fig2.add_trace(go.Bar(y=top8.index, x=top8["Medidas_Protetivas_Concedidas"], orientation="h", name="Medidas protetivas", marker=dict(color=PRIMARY), hovertemplate="%{y}<br>Medidas: %{x:,}<extra></extra>"))
...
with col3:
    ...
    fig3 = go.Figure()
    fig3.add_trace(go.Bar(y=top10.index, x=top10["Condenacoes_CNJ"], orientation="h", name="Condenações", marker=dict(color=PRIMARY), hovertemplate="%{y}<br>Condenações: %{x:,}<extra></extra>"))
    fig3.add_trace(go.Bar(y=top10.index, x=top10["Arquivamentos_CNJ"], orientation="h", name="Arquivamentos", marker=dict(color="#cfe8e6"), hovertemplate="%{y}<br>Arquivamentos: %{x:,}<extra></extra>"))
    scale2 = top10["Condenacoes_CNJ"].max() if top10["Condenacoes_CNJ"].max()>0 else 1
    fig3.add_trace(go.Scatter(y=top10.index, x=top10["Taxa_Condenacao"]*scale2, mode="lines+markers", name="Taxa condenação (esc.)", line=dict(color=ACCENT,width=2), hovertemplate="%{y}<br>Taxa cond. esc.: %{x:.2f}<extra></extra>"))
    fig3 = standard_layout(fig3, H_LARGE, left_margin=240)
    fig3.update_layout(barmode="group", yaxis=dict(autorange="reversed"), xaxis_title="Contagens")
...
with r1:
    ...
    fig4 = go.Figure(go.Bar(x=df_var["Variacao_Percentual"], y=df_var["UF"], orientation="h", marker_color=colors, hovertemplate="%{y}<br>Variação: %{x:.2f}%<extra></extra>"))
    fig4.add_vline(x=0, line=dict(color=MUTED, dash="dash"))
    fig4 = standard_layout(fig4, H_MED, left_margin=240)
    fig4.update_layout(yaxis=dict(autorange="reversed"), xaxis_title="Variação (%)")
...
with r2:
    ...
    for _,row in top15.iterrows():
        fig5.add_shape(type="line", x0=0, x1=row["Taxa_100mil_Mulheres_SINESP"], y0=row["UF"], y1=row["UF"], line=dict(color="#0b2530", width=6))
    fig5.add_trace(go.Scatter(x=top15["Taxa_100mil_Mulheres_SINESP"], y=top15["UF"], mode="markers", marker=dict(size=12, color=SECOND), hovertemplate="%{y}<br>Taxa: %{x:.2f}<extra></extra>"))
    med = df["Taxa_100mil_Mulheres_SINESP"].median()
    fig5.add_vline(x=med, line=dict(color=MUTED, dash="dash"))
    fig5 = standard_layout(fig5, H_MED, left_margin=240)
    fig5.update_layout(yaxis=dict(autorange="reversed"), xaxis_title="Taxa /100k")
...
with r3:
    ...
    fig6 = go.Figure(go.Bar(x=idx["Indice_Impunidade"], y=idx["UF"], orientation="h", marker_color=colors, hovertemplate="%{y}<br>Índice: %{x:.2f}<extra></extra>"))
    med_imp = idx["Indice_Impunidade"].median()
    fig6.add_vline(x=med_imp, line=dict(color=MUTED, dash="dash"))
    fig6 = standard_layout(fig6, H_MED, left_margin=240)
    fig6.update_layout(yaxis=dict(autorange="reversed"), xaxis_title="Índice de Impunidade")
